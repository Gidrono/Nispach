...
number_of_files = int(input('How many appendices are there? '))
filesnum = list(range(1, (number_of_files+1)))
index = 1
for file in filesnum:
    filename = str(input('What does the appendice contain? '+str(file)+'? '))
    document = Document()
    ...
    paragraph.alignment = 1
    p.add_run('Appendice ' + str(file)).bold = True
